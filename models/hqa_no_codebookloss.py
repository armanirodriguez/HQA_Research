...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        blocks = [nn.Conv2d(in_feat_dim, hidden_dim, kernel_size=3, padding=1), Mish()]#Mish()]
...
class HQA(pl.LightningModule):
    def __init__(
        ...
        self.lr = lr
        self.code_count = torch.zeros(codebook_slots)
    ...
    def training_step(self, batch, batch_size):
        ...
        recon, orig, _, _, indices, kl, commit_loss = self(x)
        ...
        code_count = self.code_count.to('cuda')
    ...
    def forward_full_stack(self, img):
        z_e = self.encode(img)
        z_q, indices, kl, commit_loss = self.codebook(z_e)
        img_recon_dist = self.decode(z_q) #z_q
