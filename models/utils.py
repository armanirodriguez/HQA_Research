...
# TRAINING
def save_img(recon, label, path, idx, file_name_suffix = ""):
    p = Path(path)
...
def test(model, dl_test):
    ...
    for x, _ in dl_test:
        img = x.to(device)       
        recon, orig, z_e_ = model.forward_full_stack(img)       
...
def interpolate(a, b, ds_test, vqvae, grid_x=16):
    ...
    for i, z_e_interpolated in enumerate(interpolate_x):       
        ...
        recon = vqvae.decode(z_e).squeeze() 
        results[i] = recon
