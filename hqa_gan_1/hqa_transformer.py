...
class VQGANTransformer(nn.Module):
    ...
    @staticmethod
    def load_hqagan(checkpoint_path):
        model = HQAGAN.load_from_checkpoint(checkpoint_path)
        model = model.eval()
        return model

    @torch.no_grad()
    def encode_to_z(self, x):
        quant_z, indices, _ = self.encode(x)
